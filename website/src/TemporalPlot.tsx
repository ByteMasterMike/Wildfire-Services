...
export function TemporalPlot({ values, labels, ceiling, compact = false, unit, onInspect }: {
  ...
  return <div ref={host} className={`temporal-plot ${compact?'is-mini':''}`}>
    <svg viewBox={`0 0 ${size.width} ${size.height}`} role="img" tabIndex={0} aria-label={`${unit}: ${labels[0]??''} to ${labels.at(-1)??''}. Arrow keys inspect periods.`}
      ...
      <path d={path} stroke="#b7a0f0" strokeWidth={compact?1.8:2} fill="none" strokeLinejoin="round"/>
      ...
      {active!==null&&active<values.length&&<g><path d={`M${x(active)} ${top}V${bottom}`} stroke="#ffffff55" strokeDasharray="3 3"/>{values[active]!==null&&<circle cx={x(active)} cy={y(values[active]!)} r="4" fill="#b7a0f0"/>}</g>}
    </svg>
  </div>;
}
