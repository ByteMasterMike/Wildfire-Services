...
export function barSvg(
    ...
    const width =
      row.value === null
        ...
        : share
          ? (percent ?? 0) * 5.4
          : (row.value / max) * 540
    const bar = `<g>${labels.map((label, i) => `<text x="24" y="${y + 15 + i * 15}" fill="#ccc" font-size="12">${escapeXML(label)}</text>`).join("")}<rect x="240" y="${center - 10}" width="540" height="20" fill="#ffffff08" rx="3"/><rect x="240" y="${center - 10}" width="${width}" height="20" fill="${
      row.value === null ? "url(#missing)" : color
    }" rx="3"/>${
      row.value === null
        ? `<text x="278" y="${center + 4}" text-anchor="middle" font-size="10" fill="#ddd">No data</text>`
        : ""
    }<text x="866" y="${center + 4}" text-anchor="end" fill="#ddd" font-size="12">${
      row.value === null
        ? "—"
        : share
          ? `${percent!.toFixed(1)}%`
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="${y + 28}" viewBox="0 0 900 ${y + 28}" font-family="Arial, sans-serif"><defs><pattern id="missing" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="8" height="8" fill="#333"/><rect width="4" height="8" fill="#666"/></pattern></defs><rect width="100%" height="100%" fill="#222"/><text x="24" y="32" fill="#eee" font-size="18">${escapeXML(title)}</text>${heading.map((line, i) => `<text x="24" y="${56 + i * 16}" fill="#aaa" font-size="11">${escapeXML(line)}</text>`).join("")}${bars.join("")}</svg>`
}
