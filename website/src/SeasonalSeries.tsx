...
export function SeasonalSeries() {
  ...
  return <div className="analysis-chart seasonal-panel">
    ...
    <div className="series-readout seasonal-readout" aria-live="polite"><span>{active?`Week ${active.week} · ${active.years.length} contributing ${active.years.length===1?'year':'years'}`:`52 weeks · ${years.length} selected ${years.length===1?'year':'years'}`}</span>{active&&<div><strong>{years.length>1?'Average: ':''}{active.mean===null?'Unavailable':active.mean.toLocaleString(undefined,{maximumFractionDigits:2})}</strong>{referenceLines.map(line=><span key={line.label} style={{color:line.color}}>{line.label}: {active.counts[Number(line.label)]??'—'}</span>)}</div>}</div>
  </div>;
}
