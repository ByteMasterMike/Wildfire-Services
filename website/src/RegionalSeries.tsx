...
import { regionalSeries } from './temporal.ts';
...
export function RegionalSeries() {
  ...
  const remote=useRemote(validation?null:JSON.stringify(['regional',filters]),()=>getRecords('epss',filters));
  const series=useMemo(()=>remote.data?regionalSeries(remote.data,filters,interval):[],[remote.data,filters,interval]);
  ...
  return <div className="analysis-chart regional-panel">
    <ExportActions disabled={!ready} rows={()=>series.flatMap(region=>region.buckets.map(bucket=>({dataset:'EPSS',division:region.name,period_start:bucket.start,period_end:bucket.end,outages:bucket.count,utility:'PG&E',county:filters.county})))} svg={()=>regionalSvg(title,caption,series)}/>
    ...
    <div className="regional-readout" aria-live="polite">{bucket?<span title={`${active!.name} · ${bucket.start} – ${bucket.end}`}><strong>{bucket.count} outages</strong> · {active!.name} · {bucket.start} – {bucket.end}</span>:<span>{ready?`${remote.data!.length.toLocaleString()} outages`:''}</span>}
